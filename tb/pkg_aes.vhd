...
package pkg_aes is
    ...
    -- AES S-box (FIPS-197), integer indexed
    constant C_AES_SBOX : t_sbox := (
    ...
    function f_sub_bytes(vec     : std_logic_vector(127 downto 0)) return std_logic_vector;
    function f_shift_rows(state  : std_logic_vector(127 downto 0)) return std_logic_vector;
    function f_xtime(b           : std_logic_vector(7 downto 0)) return std_logic_vector;
    function f_gmul2(b           : std_logic_vector(7 downto 0)) return std_logic_vector;
    function f_gmul3(b           : std_logic_vector(7 downto 0)) return std_logic_vector;
    function f_mix_columns(state : std_logic_vector(127 downto 0)) return std_logic_vector;
    function f_add_round_key(state : std_logic_vector(127 downto 0); roundkey : std_logic_vector(127 downto 0)) return std_logic_vector;
    function f_aes_round(state : std_logic_vector(127 downto 0); roundkey : std_logic_vector(127 downto 0)) return std_logic_vector;

end package pkg_aes;

package body pkg_aes is

    function f_sub_bytes(vec : std_logic_vector(127 downto 0))
        return std_logic_vector is
        variable result : std_logic_vector(127 downto 0);
        variable b      : std_logic_vector(7 downto 0);
    begin
        for i in 0 to 15 loop
            b                                      := vec(127 - i * 8 downto 120 - i * 8);
            result(127 - i * 8 downto 120 - i * 8) :=
    ...
    function f_shift_rows(state : std_logic_vector(127 downto 0))
        ...
        variable s, r   : byte_array;
        variable result : std_logic_vector(127 downto 0);
    begin
        -- unpack
        for i in 0 to 15 loop
            s(i) := state(127 - i * 8 downto 120 - i * 8);
        end loop;
        ...
        -- pack
        for i in 0 to 15 loop
            result(127 - i * 8 downto 120 - i * 8) := r(i);
        end loop;
    ...
    begin
        ...
        for c in 0 to 3 loop
            r(c * 4 + 0) :=
            f_gmul2(s(c * 4 + 0)) xor f_gmul3(s(c * 4 + 1)) xor
            s(c * 4 + 2) xor s(c * 4 + 3);

            r(c * 4 + 1) :=
            s(c * 4 + 0) xor f_gmul2(s(c * 4 + 1)) xor
            f_gmul3(s(c * 4 + 2)) xor s(c * 4 + 3);

            r(c * 4 + 2) :=
            s(c * 4 + 0) xor s(c * 4 + 1) xor
            f_gmul2(s(c * 4 + 2)) xor f_gmul3(s(c * 4 + 3));

            r(c * 4 + 3) :=
            f_gmul3(s(c * 4 + 0)) xor s(c * 4 + 1) xor
            s(c * 4 + 2) xor f_gmul2(s(c * 4 + 3));
        end loop;
    ...
    function f_add_round_key(
        ...
        roundkey : std_logic_vector(127 downto 0)
    ) return std_logic_vector is
    begin
    ...
    function f_aes_round(
        ...
        variable v_state : std_logic_vector(127 downto 0);
    begin
        -- f_sub_bytes
        v_state := f_sub_bytes(state);

        -- f_shift_rows
        v_state := f_shift_rows(v_state);

        -- f_mix_columns
        v_state := f_mix_columns(v_state);
