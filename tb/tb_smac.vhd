...
architecture tb of tb_smac_1 is
  --constant declarations
  constant C_NUM_OF_STREAMS    : integer := 1;
  constant C_IS_SMAC_N         : integer := 0;
...
begin
  ...
  begin
    ...
    if run("test_smac_1") then
      CP_LOOP_0 : for i in 0 to 1024 loop
        AD_LOOP_0 : for j in 1 to 1024 loop --j=0 case should be corrected
          v_num_of_cp_bytes := i;
          v_num_of_ad_bytes := j;

          p_prep_smac_1_packet(i + j, v_num_of_ad_bytes, v_num_of_cp_bytes, v_a1_o, v_a2_o, v_a3_o, v_inp_word_que, v_exp_word_que);
